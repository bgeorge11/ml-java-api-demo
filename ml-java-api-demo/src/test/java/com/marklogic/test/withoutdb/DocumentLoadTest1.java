...
@Configuration
@PropertySource(value = { "classpath:DocumentLoadWithoutDatabase.properties", "classpath:user.properties"}, ignoreResourceNotFound = true)
public class DocumentLoadTest1 extends AbstractApiTest {
    ...
    public void testLoadJSONDocuments() throws Exception {
    	...
    	String methodName = new DocumentLoadTest1() {}.getClass().getEnclosingMethod().getName();
    	String className = this.getClass().getName();
        String prefix = java.util.UUID.randomUUID().toString();
    	String COLLECTION_NAME = prefix + JSON_DOC_COLLECTION_SUFFIX;
    	Date start = new Date();
        System.out.println(start.toString() + " Started Test Case: " + className);
        ...
        System.out.println(new Date().toString() + " Loaded " + TOTAL_JSON_DOCS_ADDED + " documents.");
 
        countOfJsonDocs = deleteDocuments(client, COLLECTION_NAME);
        assertEquals(0, countOfJsonDocs);
        System.out.println(new Date().toString() + " Deleted " + TOTAL_JSON_DOCS_ADDED + " documents.");
        
    	Date end = new Date();
        System.out.println(end.toString() + " Ended Test Case: " + className);
        System.out.println("Execution time for " + className+ " is " + (end.getTime() - start.getTime())/1000 + " seconds.");
    }
